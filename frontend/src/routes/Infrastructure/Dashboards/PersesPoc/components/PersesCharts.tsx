...
/**
 * Perses Gauge Chart Component
 * Uses ECharts via @perses-dev/components
 */
export interface PersesGaugeChartProps {
  title: string
  value: number
  max?: number
  loading?: boolean
...
export function PersesGaugeChart({ title, value, max = 100, loading, height = 200, color }: PersesGaugeChartProps) {
  const option: EChartsOption = useMemo(
    () => ({
      series: [
        {
