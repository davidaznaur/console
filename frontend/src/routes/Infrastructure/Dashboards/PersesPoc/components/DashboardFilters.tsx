...
export function DashboardFilters({
  ...
  const renderSelectToggle = (
    ...
    <MenuToggle
      onClick={() => handleSelectToggle(type, !isOpen)}
      isExpanded={isOpen}
      isDisabled={loading}
                  ...
                  {options.availableClusters.length === 0 ? (
                    ...
                    options.availableClusters.map((cluster) => (
                      <SelectOption
                        ...
                        value={cluster}
                        hasCheckbox
                        isSelected={filters.clusters.includes(cluster)}
                      >
                        {cluster}
                      </SelectOption>
                    ))
                  ...
                  {options.availableNamespaces.length === 0 ? (
                    ...
                    options.availableNamespaces.slice(0, 50).map((ns) => (
                      <SelectOption
                        ...
                        value={ns}
                        hasCheckbox
                        isSelected={filters.namespaces.includes(ns)}
                      >
                        {ns}
                      </SelectOption>
                    ))
                ...
                <SelectList>
                  {options.availableKinds.map((kind) => (
                    <SelectOption
                      ...
                      value={kind}
                      hasCheckbox
                      isSelected={filters.kinds.includes(kind)}
                    >
                      {kind}
                    </SelectOption>
                  ))}
            ...
            {filters.searchText && (
              <FlexItem>
                <ChipGroup categoryName="Search">
                  <Chip onClick={() => onFiltersChange({ ...filters, searchText: '' })}>
                    {filters.searchText}
                  </Chip>
                </ChipGroup>
              </FlexItem>
