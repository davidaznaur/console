...
/**
 * Create a GaugeChart panel definition
 */
export function createGaugeChartPanel(
  name: string,
  description: string,
  max = 100,
  thresholds?: { value: number; color: string }[]
 ...
 */
export function createMinimalTestDashboard(): DashboardResource {
  const panels: Record<string, PanelDefinition> = {
    testStat: {
      kind: 'Panel',
      spec: {
        display: {
          name: 'Test Stat Panel',
          description: 'A simple stat panel for testing',
        },
        plugin: {
          kind: 'StatChart',
          spec: {
        ...
        items: [
          {
            ...
            width: 12,
            height: 6,
            content: { $ref: '#/spec/panels/testStat' },
          },
        ],
      },
    },
  ...
  return {
    ...
    spec: {
      display: {
        name: 'Test Dashboard',
        description: 'Minimal dashboard for testing Perses integration',
      },
      duration: '1h',
      refreshInterval: '30s',
      variables: [],
