...
export interface UseAcmSearchDashboardDataResult {
...
const RESOURCE_KINDS = ['Deployment', 'DaemonSet', 'StatefulSet', 'ReplicaSet', 'Job', 'Service', 'ConfigMap', 'Secret', 'VirtualMachine']
...
function parseKindQuery(searchText: string): { kinds: string[]; remainingText: string } {
  ...
  const detectedKinds = DEFAULT_WORKLOAD_KINDS.filter((kind) => searchText.toLowerCase().includes(kind.toLowerCase()))
 ...
 */
export function useAcmSearchDashboardData(filters: DashboardFilterState, refreshInterval?: number): UseAcmSearchDashboardDataResult {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const kindsToFetch =
        filters.kinds.length > 0 ? filters.kinds : kindsFromSearch.length > 0 ? kindsFromSearch : RESOURCE_KINDS
...
export default useAcmSearchDashboardData
