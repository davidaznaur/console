/**
 * ACM Search GraphQL service helpers for the Perses dashboard.
 * This mirrors the Search API contract and provides typed helpers.
 */
...
// Re-export types needed by the plugin
 ...
 */
export function getResourcesWithRelated(kind: string, relatedKinds: string[]): IRequestResult<ISearchResult> {
  const query: SearchQuery = {
    operationName: 'searchResult',
    variables: {
      input: [
 ...
 */
export async function getFilteredClusterSummary(dashboardFilters?: { clusters?: string[] }): Promise<ClusterSummary> {
 ...
 */
export async function getFilteredNodes(dashboardFilters?: { clusters?: string[] }): Promise<SearchResultItem[]> {
...
export async function getFilterOptions(): Promise<{
  ...
  const [clustersResult, namespacesResult] = await Promise.all([getClusters().promise, getNamespaces().promise])
